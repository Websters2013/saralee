...
/*  Copyright 2009-2011  Andrew Alba  (email : andrew.alba@gmail.com)
 ...
 * [FSR_best_of_month star_type="star"]
 * star_type is for styling purposes ("star","abuse")
 */
...
/* This was user submisson from Bob Sawyer at greendoginteractive.com
 * This will give you the all time best rated posts.
 * [FSR_all_time_best star_type="star"]
 * star_type is for styling purposes ("star","abuse")
 */
function FSR_all_time_best_func($atts) {
	...
	return $FSR->getAllTimeBest($star_type);
}
...
function FSR_all_time_best($star_type = "star") {
	global $FSR;
	echo $FSR->getAllTimeBest($star_type);
}

wp_register_style('five-star-rating-CSS', WP_PLUGIN_URL . '/five-star-rating/assets/css/five-star-rating.css');
...
function fsr_admin_warnings() {
	...
	if ( !get_option('fsr_cookie_expiration') && !isset($_POST['submit']) ) {
		function fsr_warning() {
			echo "
			<div id='fsr-warning' class='updated fade'><p><strong>".__('Five Star Rating plugin is almost ready.', 'five-star-rating')."</strong> ".sprintf(__('You must <a href="%1$s">enter your cookie expiration</a> for it to work.', 'five-star-rating'), "options-general.php?page=five-star-rating/five-star-rating.php")."</p></div>
			";
		}
...
function fsr_create_menu() {
	...
	add_options_page('FSR Plugin Settings', 'Five Star Rating', 'administrator', __FILE__, 'fsr_settings_page');
...
<form method="post" action="options.php">
    ...
    <p class="submit">
    <input type="submit" class="button-primary" value="<?php _e('Save Changes', 'five-star-rating') ?>" />
    </p>
</form>
